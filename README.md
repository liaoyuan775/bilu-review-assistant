...
原始文件不落库；标准化文档、审查结果和人工决定保存在后端 SQLite 中。真实上传与 02–07 演示案例的解析文本或图像会发送到 `.env.local` 配置的模型服务，只能使用脱敏材料。
...
真实文件上传固定使用 Qwen，不接受本地关键词降级。内置演示来自 `output/doc` 的七份真实 DOCX：01 解析文档后返回明确标识的固定模拟结果，用于快速展示；02–07 解析后走完整 Qwen 审查。旧 `local` 模式只用于读取历史 SQLite 记录，不再创建新任务。模型、响应或证据校验异常时，任务不保留部分结果。
...
`verify:model` 使用 02 号脱敏 DOCX 验证当前模型能否返回完整的 7 条结果。异常时只输出稳定错误码和非敏感状态，不打印 API Key 或请求正文。
